...
namespace xlog {
class xlogger : public spdlog::logger {
 public:
  explicit xlogger(std::string name);
 ...
 */
struct xdaily_filename_calculator {
  ...
  static filename_t calc_filename(const filename_t &filename,
    ...
    std::conditional<std::is_same<filename_t::value_type, char>::value,
                     memory_buf_t, fmt::memory_buffer>::type w;
 ...
 */
template <typename Mutex, typename FileNameCalc = xdaily_filename_calculator>
class xdaily_file_sink final : public base_sink<Mutex> {
 public:
  // create daily file sink which rotates on given time
  xdaily_file_sink(filename_t base_filename, int rotation_hour,
 ...
 protected:
  void sink_it_(const details::log_msg &msg) override {
    ...
    if (time >= rotation_tp_) {
      file_helper_.open(
          FileNameCalc::calc_filename(base_filename_, now_tm(time)), truncate_);
      rotation_tp_ = next_rotation_tp_();
    }
...
using xdaily_file_sink_mt = xdaily_file_sink<std::mutex>;
using xdaily_file_sink_st = xdaily_file_sink<details::null_mutex>;
